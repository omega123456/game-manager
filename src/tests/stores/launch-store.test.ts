...
describe('launch-store', () => {
  ...
  it('advances through lifecycle phases from events', () => {
    ...
    expect(useLaunchStore.getState().elapsedSeconds).toBe(3)
  ...
  it('ticks the counter only during ticking phases', () => {
  ...
  it('keeps the local elapsed value when a phase event omits elapsedSeconds', () => {
    const { applyLifecycle, tick } = useLaunchStore.getState()
    applyLifecycle(lifecycle({ phase: 'waitingForProcess', elapsedSeconds: 4 }))
    tick()
  ...
  it('isTickingPhase reports the running phases', () => {
    expect(isTickingPhase('waitingForProcess')).toBe(true)
