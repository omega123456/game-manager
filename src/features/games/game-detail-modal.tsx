...
export function GameDetailModal(): React.JSX.Element {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(nextOpen) => (!nextOpen ? closeDetailModal() : undefined)}>
      {isOpen ? <GameDetailModalInner key={`${selectedGameId ?? 'none'}-open`} selectedGameId={selectedGameId} /> : null}
    </Dialog>
  )
}
        ...
        <DialogHeader className="gap-3">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              <DialogDescription>
                Tune launch details now. Script assignment and launch orchestration land in later phases.
              </DialogDescription>
            </div>
            <Button type="button" variant="ghost" size="icon" onClick={closeDetailModal} aria-label="Close game detail">
          ...
          <div className="max-h-[calc(90vh-10rem)] overflow-y-auto px-6 py-6">
            <TabsContent value="overview" className="mt-0">
              {gameQuery.isLoading || !game || !meta ? (
                <div className="grid gap-6 lg:grid-cols-[19rem_1fr]" data-testid="game-detail-loading">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                      {Array.from({ length: 3 }).map((_, index) => (
                        <div key={index} className="h-28 animate-pulse rounded-[1.4rem] bg-surface-high" />
                      ))}
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="space-y-6">
                    <section className="overflow-hidden rounded-[1.8rem] border border-border bg-surface-container">
                      <div className="border-b border-border bg-gradient-to-r from-primary/20 via-secondary/10 to-transparent px-6 py-5">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                          <div className="space-y-2">
                            ...
                            <p className="max-w-2xl text-sm text-muted-foreground">
                              Launch wiring is staged next. This overview gives you the art, stats, and configuration summary first.
                            </p>
                          </div>
                      ...
                      <div className="space-y-4 px-6 py-5">
                        <div className="rounded-[1.4rem] border border-border bg-background/70 p-4">
                          ...
                          <p className="mt-2 break-all text-sm text-foreground">{game.launchTarget}</p>
                          {game.arguments ? (
                            <>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-foreground">
                                {game.monitorMode === 'named' ? 'Launcher-aware monitoring' : 'Direct executable monitoring'}
                              </p>
            ...
            <TabsContent value="scripts" className="mt-0">
              ...
              >
                <div className="flex max-w-2xl items-start gap-4">
                  ...
                  <div className="space-y-2">
                    <h2 className="font-heading text-xl font-semibold text-foreground">Scripts land after assignments exist</h2>
                    <p className="text-sm text-muted-foreground">
                      Script assignment available after Scripts/Groups are set up. This tab will show direct assignments,
                      inherited scripts, and resolved order in Phase D3.
                    </p>
                  </div>
                </div>
              </section>
